    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-6 py-6 border-b border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-gradient-to-tr from-indigo-500 to-violet-500 rounded-lg shadow-lg shadow-indigo-500/20"></div>
            <div>
           ...
           <div className="flex justify-between items-end">
             ...
             {props.onEnhancePrompt && (
                <button 
                  ...
                  className="text-[10px] font-medium text-indigo-400 hover:text-indigo-300 transition-colors disabled:opacity-50 flex items-center gap-1 mb-2"
                >
           ...
           <div className="relative group">
                ...
                <div className="absolute bottom-2 right-2">
                    <button 
                        ...
                        className={`p-1.5 rounded-lg transition-all ${props.hasHeroRef ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/25' : 'bg-zinc-800 text-zinc-500 hover:text-zinc-300'}`}
           ...
           {props.hasHeroRef && (
              <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                  <label className="block w-full cursor-pointer group">
                      <div className="w-full h-32 rounded-xl border border-dashed border-white/10 hover:border-indigo-500/50 bg-white/5 flex flex-col items-center justify-center transition-all relative overflow-hidden">
                          {props.heroImage ? (
                              <img src={`data:${props.heroImage.mimeType};base64,${props.heroImage.base64}`} className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:opacity-40 transition-opacity" />
                          ) : (
                              <div className="text-zinc-500 group-hover:text-indigo-400 transition-colors flex flex-col items-center gap-2">
        ...
        <div className="space-y-5">
            ...
            <div className="flex items-center justify-between py-1">
                ...
                <button 
                  ...
                  className={`w-10 h-6 rounded-full transition-colors relative ${props.includeText ? 'bg-indigo-600' : 'bg-zinc-800'}`}
                >
                  <div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full shadow-md transform transition-transform ${props.includeText ? 'translate-x-4' : 'translate-x-0'}`} />
                </button>
            </div>
        </div>
        ...
        <div className="space-y-4">
                ...
                <div className="flex gap-2 flex-wrap">
                    {PAGE_SIZES.map(s => (
                        <button
                            ...
                            className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-all ${props.pageSize === s.id ? 'bg-white text-zinc-950 border-white' : 'bg-transparent text-zinc-500 border-white/10 hover:border-white/20'}`}
                        >
                            {s.label}
                        </button>
                    ))}
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                  ...
                  <span className="text-xs font-bold text-indigo-400">{props.pageAmount} Pages</span>
              </div>
              <input 
